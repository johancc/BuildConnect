...
export const createNewUser = async (values) => {
  ...
  try {
    ...
    // Remove from firebase if creation failed somehow.
    if (tokenId) await post('/api/removeUser', {token: tokenId});
...
export const createNewMentor = async (values) => {
  ...
  try {
    ...
    // Remove from firebase if creation failed somehow.
    if (tokenId) await post('/api/removeMentor', {token: tokenId});
...
export const requestToJoin = async (message, projectID, tokenId) => {
  return post('/api/requestToJoin',
      {message: message, projectID: projectID, token: tokenId});
};

export const requestMentor = async (message, mentor, tokenId) => {
  return post('/api/requestMentor',
      {message: message, mentor: mentor, token: tokenId});
};
...
export const getProjectOwner = async (project, tokenId) => {
  const owner = await get('/api/owner',
      {ownerID: project.projectOwner, token: tokenId});
  return owner;
};
...
export const getProject = async (projectID, tokenId) => {
  const projectData = await get('/api/project', {_id: projectID, token: tokenId});
  return projectData;
};
